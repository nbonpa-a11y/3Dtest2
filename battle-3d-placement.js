...
(function(root){'use strict';
 ...
 function numberAnchor(THREE,entry,camera){const b=body(entry),point=entry.model.root.position.clone();point.y+=b.height;
  const forward=camera.position.clone().sub(point).normalize();return point.addScaledVector(forward,b.radius);
 }
 ...
 // GSssFont::GetCharacterHeight reads SSFB +0x1c (48), not line height (61).
 function numberEm(critical,unit){const panes=root.RankBattleNativeLayout?.layouts['423']?.panes;const size=panes?.[critical?'Text_Damage01':'Text_Damage']?.fontSize||(critical?44:37);return 48*Math.floor(size*100/48)/100*unit;}
