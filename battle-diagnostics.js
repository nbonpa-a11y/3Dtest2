(function(root){'use strict';
 ...
 function watchSlowFrames(send){try{if(root.PerformanceObserver?.supportedEntryTypes?.includes('long-animation-frame')){
  const observer=new root.PerformanceObserver(list=>{for(const e of list.getEntries())send('slow-frame',{
   startMs:+e.startTime.toFixed(2),durationMs:+e.duration.toFixed(2),blockingMs:+(e.blockingDuration||0).toFixed(2),
   layoutMs:e.styleAndLayoutStart?+Math.max(0,e.startTime+e.duration-e.styleAndLayoutStart).toFixed(2):0,
   scripts:Array.from(e.scripts||[]).sort((a,b)=>b.duration-a.duration).slice(0,6).map(v=>({file:String(v.sourceURL||'').split(/[\\/]/).pop().split(/[?#]/)[0].slice(0,100),function:String(v.sourceFunctionName||'').slice(0,100),durationMs:+v.duration.toFixed(2),forcedLayoutMs:+(v.forcedStyleAndLayoutDuration||0).toFixed(2),invoker:String(v.invokerType||'').slice(0,60)}))
 ...
 const report=()=>({version:4,userAgent:root.navigator?.userAgent||'',savedAt:new Date().toISOString(),records});
 function save(){try{root.localStorage?.setItem(key,JSON.stringify(report()));}catch{}}
 root.RankBattleDiagnosticsPanel={receive(data){records.push(data);if(records.length>300)records.shift();if(data.kind==='sample'){const stages=Object.entries(data.cpuMs).map(([k,v])=>k+': '+v+' ms').join(' / ');output.textContent='FPS: '+data.fps+' / 表示キャラ: '+data.visibleActors+'\nキャラ描画命令等: '+data.drawCalls+' / 三角形: '+data.triangles+'\n'+stages+'\nテクスチャ数: '+data.textures+' / ジオメトリ数: '+data.geometries+' / JSヒープ: '+(data.jsHeapMiB??'取得不可')+' MiB\nエフェクト: '+data.effectMode+' / キャラ変形停止: '+data.frozen+' / 再生エフェクト: '+(data.activeEffects??'不明')+'\nGPU時間: '+(data.gpu?.lastMs??'取得不可')+' ms / 長時間処理: '+(data.longTasks?.maxMs??0)+' ms\n最長フレーム: '+(data.frameMs?.max??'不明')+' ms / GPU設定読み戻し: '+(data.effectStateReadback===false?'省略':'あり・不明')+'\n描画CPU時間はGPU完了時間ではありません。メモリーはGPUやブラウザー全体を含みません。';}else if(['error','context-lost','context-restored'].includes(data.kind))output.textContent+='\n'+data.kind+': '+(data.message||'');if(['error','context-lost'].includes(data.kind)||Date.now()-lastSaved>=5000){lastSaved=Date.now();save();}}};
 watchSlowFrames((kind,data)=>root.RankBattleDiagnosticsPanel.receive({kind,time:Date.now(),surface:'parent',...data}));
