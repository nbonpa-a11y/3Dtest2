...
async function playRound(result,round){
 ...
 const motionsOn=motionEnabled();motionCardState=motionsOn?new Map(state.parties.flatMap((p,side)=>p.members.flatMap((a,slot)=>a?[[side+':'+slot,{hp:a.hp,conditions:structuredClone(a.conditions)}]]:[]))):null;if(motionsOn&&globalThis.RankBattle3D)RankBattle3D.onCards=updates=>{if(finishPlayback||!motionCardState)return;const changed=new Set();for(const u of updates||[])if(motionCardState.has(u.key)){const before=motionCardState.get(u.key),after=RankBattleMotion.applyCardUpdate(before,u);motionCardState.set(u.key,after);if(before.hp!==after.hp||JSON.stringify(before.conditions)!==JSON.stringify(after.conditions))changed.add(Number(u.key.split(':')[0]));}for(const side of changed)renderRoster(side);};render();if(motionsOn)await globalThis.RankBattle3D?.prepare();
