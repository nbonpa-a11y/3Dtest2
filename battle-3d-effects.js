...
(function(root){'use strict';
 ...
 root.createBattleEffects=async function(renderer,loadScript){
  ...
  return {
   ...
   update(dt){if(!context||disposed)return;for(const item of active){if(!item.resolve)continue;const t=item.resolve();item.handle.setLocation?.(t.position.x,t.position.y,t.position.z);item.handle.setRotation?.(...t.rotation);}let remaining=Math.max(0,dt)*60;while(remaining>0){const step=Math.min(1,remaining);context.update(step);remaining-=step;}},
   draw(camera){if(!context||disposed)return;context.setProjectionMatrix(camera.projectionMatrix.elements);context.setCameraMatrix(camera.matrixWorldInverse.elements);context.draw();if(renderer.resetState)renderer.resetState();else renderer.state?.reset();},
