...
(function(root){'use strict';
 ...
 root.createBattleEffects=async function(renderer,loadScript){
  ...
  return {
   async prepare(actors){const ids=new Set([...Object.values(manifest.conditions||{}),...Object.values(manifest.reactions||{}),...Object.values(manifest.slip||{})].flatMap(d=>d.effects||[]));for(const uid of [0x8602009b,...actors.map(a=>Number(a.actionUid))])for(const d of manifest.actions[uid]||[])for(const id of d?.effects||[])ids.add(id);for(const id of ids)await load(id);},
   play(ids,position,placements=[],resolve){if(disposed)return;for(const [index,id] of (ids||[]).entries()){const effect=ready.get(id);if(!effect||disposed)continue;const info=placements[index]?.asset===id?placements[index]:placements.find(p=>p.asset===id);const transform=info&&resolve?resolve(info):{position,scale:1,rotation:[0,0,0]},at=transform.position;const handle=context.play(effect,at.x,at.y,at.z);if(handle){handle.setScale?.(transform.scale,transform.scale,transform.scale);handle.setRotation?.(...transform.rotation);active.push({handle,resolve:info&&info.attach!==0&&resolve?()=>resolve(info):null});}}},
   ...
   draw(camera){if(!context||disposed||!active.length)return;context.setProjectionMatrix(camera.projectionMatrix.elements);context.setCameraMatrix(camera.matrixWorldInverse.elements);try{context.draw();}finally{if(resetRenderer)renderer.resetState();else renderer.state?.reset();}},
